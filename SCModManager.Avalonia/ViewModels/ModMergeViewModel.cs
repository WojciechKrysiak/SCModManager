...
namespace SCModManager.Avalonia.ViewModels
{
	public class ModMergeViewModel : DialogViewModel<Tuple<MergedMod, bool>>
    {
        ...
        private void CheckFileResolved()
        {
            ...
            if (selected != null)
            {
                if (!ReferenceHasConflicts(_selected.File))
                {
                    _currentProcesses.Remove(_selected.File);
                    SelectedModFile = null;
                }

                UpdateModList(selected.File.Path);
            }
        ...
        public MergeProcessViewModel CurrentProcess
        {
            get
            {
                if (_selected != null && ReferenceHasConflicts(_selected.File))
                {
                    ...
                    return _currentProcesses[_selected.File];
                }
                return null;
